...
async fn ensure_tenant_role(admin: &PgPool) {
    ...
    // so the test's meaning doesn't depend on a server default.
    sqlx::query(
        r#"DO $$
        BEGIN
          IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'bulkops_tenant') THEN
            CREATE ROLE bulkops_tenant LOGIN PASSWORD 'bulkops_tenant' NOBYPASSRLS;
          END IF;
        END
        END $$"#,
    )
    .execute(admin)
    .await
    .expect("create bulkops_tenant role");

    sqlx::query("GRANT USAGE ON SCHEMA bulkops TO bulkops_tenant")
        .execute(admin).await.expect("grant schema bulkops");
    ...
    sqlx::query("GRANT USAGE ON SCHEMA public TO bulkops_tenant")
        .execute(admin).await.expect("grant schema public");
    sqlx::query("GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA bulkops TO bulkops_tenant")
        .execute(admin).await.expect("grant table crud");
...
async fn tenant_pool() -> PgPool {
    PgPool::connect(&tenant_dburl()).await.expect("connect as bulkops_tenant")
}
...
async fn tis1_own_tenant_run_succeeds() {
    ...
    let sum = BulkWriteService::new(tenant.clone())
        ...
        .await
        .expect("own-tenant run must succeed under the non-superuser role");
