//! The write-path seam against the REAL backbone-lead module. A bulk lead-import batch drives lead's
//! `create_lead` for each item — never raw SQL, never bypassing lead's invariants. Proves the batch lands
//! real records through the module's own write path. ZERO normal Cargo edge — the target module is reached
//! through the `BulkTargetPort`, a dev-dependency only in the test.
...
// BSEAM-1 — a bulk lead import creates REAL leads, one per item, through the lead module's write path.
#[tokio::test]
async fn bseam1_bulk_import_creates_real_leads() {
    let pool = pool().await;
    let company = Uuid::new_v4();
    let svc = BulkWriteService::new(pool.clone());
    let target = RealLeadTarget::new(pool.clone());
    ...
    let n: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM lead.leads WHERE company_id=$1 AND status='new'::lead_status")
    ...
    for (ty, id) in &refs {
        assert_eq!(ty, "lead");
        let exists: i64 = sqlx::query_scalar("SELECT count(*) FROM lead.leads WHERE id=$1")
