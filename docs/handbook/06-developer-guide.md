...
metaphor migration generate Invoice bulk-ops
metaphor migration run
...
(`bulk-ops` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed bulk-ops          # run Rust seeders
metaphor migration generate-seeds bulk-ops  # emit SQL seed files
```
